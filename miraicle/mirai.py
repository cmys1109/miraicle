...
class Mirai(metaclass=Singleton):
    ...
    def __handle_msg_origin(self, msg_origin, msg_type):
        ...
        elif msg_type in ['GroupRecallEvent', 'MemberCardChangeEvent', 'NewFriendRequestEvent',
    ...
    def handle_new_friend_request(self, event: NewFriendRequestEvent, operate: int, message: str = ''):
        """处理新好友请求
        :param
        ...
        content = {'sessionKey': self.session_key,
                   ...
                   'message': message}
        if self.adapter == 'http':
            response = self.__session.post(url=f'{self.base_url}/resp/newFriendRequestEvent', json=content).json()
        else:
            response = self.__ws_send('resp_newFriendRequestEvent', content=content)
    ...
    def handle_member_join_request(self, event: MemberJoinRequestEvent, operate: int, message: str = ''):
        ...
        content = {'sessionKey': self.session_key,
                   ...
                   'message': message}
        if self.adapter == 'http':
            response = self.__session.post(url=f'{self.base_url}/resp/memberJoinRequestEvent', json=content).json()
        else:
            response = self.__ws_send('resp_memberJoinRequestEvent', content=content)
    ...
    def handle_bot_invited_join_group_request(self, event: BotInvitedJoinGroupRequestEvent, operate: int,
        ...
        content = {'sessionKey': self.session_key,
                   ...
                   'message': message}
        if self.adapter == 'http':
            response = self.__session.post(url=f'{self.base_url}/resp/botInvitedJoinGroupRequestEvent',
                                           json=content).json()
        else:
            response = self.__ws_send('resp_botInvitedJoinGroupRequestEvent', content=content)
